...
def plot(bench, trajectories):
    ...
    for i, (label, trajectory) in enumerate(trajectories.items()):
        if label == "true_trajectory":
            axs[2].plot(np.linspace(0, 10, 101), trajectory.states[:, 1], label=pretty_name[label], color=colors[label], linestyle="dashed")
        else:
            axs[2].plot(np.linspace(0, 10, 101), trajectory.states[:, 1], label=pretty_name[label], color=colors[label])
        axs[2].set_xlim(0, 10)
        axs[2].legend()

    figs[2].savefig(f'/results/figure2.pdf')
